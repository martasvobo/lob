...
def create_bar_chart_from_csv(csv_filename):
    ...
    plt.title(
        f"Orders by Latency Histogram - mean={average_time:.1f}ns ({1000000000/average_time:,.0f} orders/s)"
    )
    ...
    stats = (
        filtered_df.groupby("Order Type")["Times"]
        .agg(["mean", lambda x: x.quantile(0.15), lambda x: x.quantile(0.85)])
        .reset_index()
    )
    ...
    stats = (
        market_df.groupby("Executed Orders")["Times"]
        .agg(["mean", "count"])
        .reset_index()
    )
    ...
    stats = (
        balance_df.groupby("AVL Tree Balances")["Times"]
        .agg(["mean", "count"])
        .reset_index()
    )
    ...
    stats = (
        market_df.groupby(["Executed Orders", "AVL Tree Balances"])["Times"]
        .agg(["mean", "count"])
        .reset_index()
    )
