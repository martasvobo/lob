...
public:
  ...
  // Optimized pop for single consumer - no compare_exchange needed
  void pop(T &v) noexcept {
    auto const currentTail = tail_;
    auto &slot = slots_[idx(currentTail)];
    while (turn(currentTail) * 2 + 1 != slot.turn.load(std::memory_order_acquire))
      ;
    v = slot.move();
    slot.destroy();
    slot.turn.store(turn(currentTail) * 2 + 2, std::memory_order_release);
    tail_ = currentTail + 1;  // No atomic needed for single consumer
  }

  // Optimized try_pop for single consumer
  bool try_pop(T &v) noexcept {
    auto const currentTail = tail_;
    auto &slot = slots_[idx(currentTail)];
    if (turn(currentTail) * 2 + 1 == slot.turn.load(std::memory_order_acquire)) {
      v = slot.move();
      slot.destroy();
      slot.turn.store(turn(currentTail) * 2 + 2, std::memory_order_release);
      tail_ = currentTail + 1;  // No atomic needed for single consumer
  ...
  /// Since this is a concurrent queue the size is only a best
  /// effort guess until all producer threads have been joined.
  ptrdiff_t size() const noexcept {
    return static_cast<ptrdiff_t>(head_.load(std::memory_order_relaxed) - tail_);
  }

  /// Returns true if the queue is empty.
  /// Since this is a concurrent queue this is only a best effort guess
  /// until all producer threads have been joined.
...
private:
  ...
  alignas(hardwareInterferenceSize) size_t tail_;  // No atomic needed for single consumer
};
} // namespace mpsc

template <typename T,
          typename Allocator = mpsc::AlignedAllocator<mpsc::Slot<T>>>
